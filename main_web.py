...
Phone Agent  - 智能版 v1.3.5 -第2026次迭代
...
--------------------3.保持所有样式、功能、布局完全不变
--------------------4.优化TTS模块，自定义GPT-SoVITS实现静默输出，提升体验
--------------------5.优化流式输出，统一执行输出的格式
1.3.3  （2026.03.06）1.优化 langchain 添加callbacks 模块
--------------------2.增加日志管理
--------------------3.优化多模态指令输出重复问题
1.3.4  （2026.03.06）1.双入口：a.main.py（GUI） b.main_web.py（web）
--------------------2.添加web入口和支持
--------------------3.优化web端的ui设计和性能
1.3.5  （2026.03.22）1.添加markitdown依赖用于文档解析
--------------------2.修复清空历史按钮点击区域异常问题
--------------------3.修复文件上传Toast提示和自动清空文件管理卡片
--------------------4.将os.path路径操作统一重构为pathlib模块
--------------------5.优化类型标注，使用现代化Python类型语法
--------------------6.加局域网IP显示和外部访问说明
