...
class ActionHandler:
    ...
    def execute(
        ...
        try:
            # 执行动作时传递完整action（包含task_type和instruction）
            return handler_method(action, screen_width, screen_height)
        except Exception as e:
            return ActionResult(
                success=False, should_finish=False, message=f"Action failed: {e}"
    ...
    def _handle_swipe(self, action: dict, width: int, height: int) -> ActionResult:
        ...
        if not start or not end:
            ...
            # 获取当前任务上下文（从action中传递的任务类型）
            task_type = action.get("task_type", "")
            is_chat_task = "聊天" in task_type or "发消息" in task_type
            has_swipe_instruction = "滑动" in action.get("instruction", "")

            # 核心限制：聊天任务中若无明确滑动指令，禁止滑动
            if is_chat_task and not has_swipe_instruction:
                return ActionResult(
                    False,
                    False,
                    "禁止无指令滑动：聊天任务中仅允许根据明确指令滑动"
                )
