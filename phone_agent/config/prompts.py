...
5. 如果当前页面找不到商品、目标联系人、店铺等信息，可以尝试 Swipe 滑动查找。
...
18. 聊天任务中（包括单次回复和持续回复），仅当用户明确指令包含"滑动"时，才能允许执行上下滑动操作。
19. 不要向上滚动或滑动
20. 第一次关闭键盘失败时可以直接使用Back
"""
)
#18/19/20/  15. 在执行任务中如果遇到滑动不生效的情况，请调整一下起始点位置，增大滑动距离重试，如果还是不生效，有可能是已经滑到底了，请继续向反方向滑动，直到顶部或底部，如果仍然没有符合要求的结果，请跳过这一步继续任务，并在finish message说明但没找到要求的项目。